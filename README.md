...
2. **整点对齐**：所有数据时间戳对齐到整30秒（`00:00:00`, `00:00:30`, `00:01:00`...），保证数据间隔均匀
...
1. **锚点选择**：用户选择多个 CO2 **变化率稳定**的时段作为校准锚点
   - 这些时段的特征是：生物活动处于稳态，CO2 变化应呈线性趋势（斜率恒定）
   - **典型锚点示例**：
     - 纯呼吸期（夜间无光，只有呼吸作用，CO2 应线性上升）
     - 光合-呼吸平衡期（稳定光照下的动态平衡，CO2 应基本持平或线性变化）
   - 锚点时段内的**实测斜率**与**理论期望斜率**之差，即为该时段的漂移速率
